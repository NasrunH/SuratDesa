...
@section('page-subtitle', 'Ubah data warga atau perangkat desa')
...
<div class="max-w-4xl mx-auto space-y-6 animate-fade-in-up">
    ...
    <div class="flex items-center justify-between">
        ...
        <h2 class="text-lg font-bold text-gray-800">Form Ubah Pengguna</h2>
    </div>

    <!-- Form Container -->
    <div class="bg-white rounded-3xl border border-slate-200 shadow-md p-8">
        <form action="{{ route('staff.penduduk.update', $penduduk->id_penduduk) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="nik" id="nik" value="{{ old('nik', $penduduk->nik) }}" required maxlength="16" minlength="16" placeholder="Masukkan 16 digit NIK"
                ...
                <div>
                    ...
                    <input type="text" name="nama" id="nama" value="{{ old('nama', $penduduk->nama) }}" required placeholder="Nama lengkap sesuai KTP"
                ...
                <div>
                    ...
                    <input type="email" name="email" id="email" value="{{ old('email', $penduduk->email) }}" placeholder="warga@example.com"
                ...
                <div>
                    ...
                    <input type="text" name="no_hp" id="no_hp" value="{{ old('no_hp', $penduduk->no_hp) }}" placeholder="Contoh: 081234567890"
                ...
                <div>
                    ...
                    <select name="role" id="role" required onchange="toggleRoleFields()"
                        ...
                        <option value="warga" {{ old('role', $penduduk->role) === 'warga' ? 'selected' : '' }}>Warga</option>
                        <option value="staff" {{ old('role', $penduduk->role) === 'staff' ? 'selected' : '' }}>Staff Desa</option>
                        <option value="kades" {{ old('role', $penduduk->role) === 'kades' ? 'selected' : '' }}>Kepala Desa</option>
                    </select>
                ...
                <div>
                    ...
                    <select name="status_akun" id="status_akun" required
                        ...
                        <option value="aktif" {{ old('status_akun', $penduduk->status_akun) === 'aktif' ? 'selected' : '' }}>Aktif</option>
                        <option value="pending" {{ old('status_akun', $penduduk->status_akun) === 'pending' ? 'selected' : '' }}>Pending</option>
                        <option value="nonaktif" {{ old('status_akun', $penduduk->status_akun) === 'nonaktif' ? 'selected' : '' }}>Non-Aktif / Ditangguhkan</option>
                    </select>
                ...
                <!-- Foto KTP -->
                <div id="ktp_field">
                    <label for="foto_ktp" class="block text-sm font-semibold text-gray-700 mb-2">Foto KTP Baru (Kosongkan jika tidak diubah)</label>
            ...
            <div>
                ...
                <textarea name="alamat" id="alamat" rows="3" placeholder="Nama Jalan, RT/RW, Dusun, Desa"
                          class="w-full px-4 py-2.5 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-teal-400/50 focus:border-teal-400 transition @error('alamat') border-red-300 bg-red-50/20 @enderror">{{ old('alamat', $penduduk->alamat) }}</textarea>
            ...
            <div id="staff_fields" class="hidden border-t border-slate-100 pt-6 space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" name="nip" id="nip_staff" value="{{ old('nip', $penduduk->staffDesa->nip ?? '') }}" placeholder="Masukkan NIP Staff"
                    ...
                    <div>
                        ...
                        <input type="text" name="jabatan" id="jabatan" value="{{ old('jabatan', $penduduk->staffDesa->jabatan ?? '') }}" placeholder="Contoh: Kasi Pelayanan"
                               class="w-full px-4 py-2.5 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-teal-400/50 focus:border-teal-400 transition">
            ...
            <div id="kades_fields" class="hidden border-t border-slate-100 pt-6 space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" name="nip_kades" id="nip_kades" value="{{ old('nip', $penduduk->kepalaDesa->nip ?? '') }}" placeholder="Masukkan NIP Kepala Desa"
                               class="w-full px-4 py-2.5 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-teal-400/50 focus:border-teal-400 transition">
                    </div>
                    <div>
                        ...
                        <input type="text" name="periode_jabatan" id="periode_jabatan" value="{{ old('periode_jabatan', $penduduk->kepalaDesa->periode_jabatan ?? '') }}" placeholder="Contoh: 2024 - 2030"
                               class="w-full px-4 py-2.5 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-teal-400/50 focus:border-teal-400 transition">
            ...
            <div class="flex items-center justify-end gap-3 border-t border-slate-150 pt-6">
                ...
                <button type="submit" class="px-6 py-2.5 bg-teal-600 hover:bg-teal-700 text-white rounded-xl transition font-semibold text-sm shadow-md shadow-teal-600/10">Simpan Perubahan</button>
            </div>
        </form>
    </div>
</div>
